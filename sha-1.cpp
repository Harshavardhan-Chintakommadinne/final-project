...
void SHA256::transform(const uint8_t* chunk) {
    ...
    for (int i = 0; i < 64; ++i) {
        uint32_t t1 = htemp + (rotr(e, 6) ^ rotr(e, 11) ^ rotr(e, 25)) + choose(e, f, g) + K[i] + w[i];
...
std::string SHA256::final() {
    ...
    while (!buffer.empty()) {
        transform(buffer.data());
        buffer.erase(buffer.begin(), buffer.begin() + std::min<size_t>(64, buffer.size()));
    }
...
int main() {
    ...
    if (!file.is_open()) {
        cerr << "Error: Could not open mark_rsv.txt" << endl;
        return 1;
    }
    ...
    cout << "Computing SHA-256 for Book of Mark (RSV)..." << endl;
